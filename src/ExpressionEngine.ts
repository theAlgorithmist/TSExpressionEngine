...
/**
 * Typescript Math Toolkit: Parse and evaluate simple numerical and general boolean expressions in infix notation.
...
export class TSMT$ExpressionEngine
{
   ...
   *
   * @returns {nothing}.  Call parse() to parse a new function followed by evaluate() for one or more
   * function evaluations.  This is only truly necessary if making a change in the independent variable
   * list before a parse.
   */
  public clear(): void
  {
   ...
   *
   * @returns {boolean} True if parsing was successful.  The expression must be parsed before evaluation.  Input
   * errors result in no parsed result and the expression may not be evaluated.
   *
   * Note:  Any prior expression stack is overwritten
   */
   ...
   *
   * @returns Array<string> Copy of the expression stack (call after parsing) for advanced applications
   */
   ...
   *
   * @param {Array<expressionOperand>} variables Corresponding values of each independent variable
   *
   ...
   *
   * @returns {expressionValue} Expression value of false if there is an error during function evaluation.  Most
   * common errors are not parsing an expression before evaluation and mismatch between variable list and input values.
   */
